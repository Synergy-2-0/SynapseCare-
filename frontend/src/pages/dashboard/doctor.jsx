...
const Badge = ({ children, variant = 'primary' }) => {
    ...
    return (
        <span className={`px-2 py-0.5 rounded-lg text-[10px] font-black border ${variants[variant]} uppercase tracking-widest`}>
            {children}
        </span>
    );
};
...
const DoctorDashboard = () => {
    ...
    const handlePrescriptionSubmit = async (e) => {
        e.preventDefault();
        if (!prescForm.appointmentId) { alert("Please select an appointment first"); return; }
        setSubmitting(true);
        try {
                                    ...
                                    {appointments.length > 0 ? appointments.map((appt, i) => (
                                        <div key={i} className="p-8 bg-slate-50 rounded-[2.5rem] flex justify-between items-center border border-slate-100 group hover:border-indigo-600 transition-all italic font-bold shadow-sm">
                                            <div className="flex items-center gap-8 italic">
                                                <div className="w-16 h-16 bg-white border border-slate-200 text-indigo-600 rounded-[1.5rem] flex items-center justify-center font-black italic shadow-lg">#{appt.tokenNumber || 'Q'}</div>
                                                <div>
                                                    <div className="text-xl font-black italic italic font-bold text-slate-900 leading-none mb-2">Patient: #{appt.patientId}</div>
                                                    <div className="flex gap-2 items-center">
                                                        <Badge variant={appt.status === 'CONFIRMED' ? 'success' : 'primary'}>{appt.status}</Badge>
                                                        <div className="text-[10px] text-slate-500 font-bold italic italic font-bold uppercase tracking-wider">{appt.date} • {appt.time}</div>
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="flex gap-4 italic font-bold items-center">
                                                {appt.status === 'PENDING' && (
                                                    <>
                                                        <button onClick={() => handleApptAction(appt.id, 'accept')} className="p-3 bg-emerald-100 text-emerald-700 rounded-xl shadow-md italic hover:bg-emerald-600 hover:text-white transition-all"><CheckCircle className="w-6 h-6 italic" /></button>
                                                        <button onClick={() => handleApptAction(appt.id, 'reject')} className="p-3 bg-rose-100 text-rose-700 rounded-xl shadow-md italic hover:bg-rose-600 hover:text-white transition-all"><XCircle className="w-6 h-6 italic" /></button>
                                                    </>
                                                )}
                                                {appt.status === 'CONFIRMED' && (
                                                    <button 
                                                        onClick={() => {
                                                            setPrescForm({ ...prescForm, appointmentId: appt.id, patientId: appt.patientId });
                                                            setActiveTab('prescriptions');
                                                        }}
                                                        className="px-5 py-2.5 bg-indigo-600 text-white rounded-xl text-xs font-black italic italic font-bold flex items-center gap-2 hover:bg-indigo-700 transition-all"
                                                    >
                                                        <ClipboardList className="w-4 h-4 italic" /> Issue Prescription
                                                    </button>
                                                )}
                                            </div>
                                        </div>
                        ...
                        {activeTab === 'prescriptions' && (
                            <div className="bg-white rounded-[4rem] p-16 shadow-2xl border border-slate-100 italic">
                                <div className="max-w-3xl mx-auto space-y-12 italic font-bold">
                                    ...
                                    <form onSubmit={handlePrescriptionSubmit} className="bg-slate-50 p-12 rounded-[3rem] border border-slate-100 space-y-8 italic">
                                        <div className="grid grid-cols-2 gap-6">
                                            <div className="space-y-3 font-bold italic">
                                                ...
                                                <input required value={prescForm.medicineName} onChange={e => setPrescForm({...prescForm, medicineName: e.target.value})} className="w-full bg-white border-2 border-slate-100 rounded-2xl p-4 font-black" placeholder="e.g. Amoxicillin" />
                                            </div>
                                            <div className="space-y-3 font-bold italic">
                                                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Dosage</label>
                                                <input required value={prescForm.dosage} onChange={e => setPrescForm({...prescForm, dosage: e.target.value})} className="w-full bg-white border-2 border-slate-100 rounded-2xl p-4 font-black" placeholder="e.g. 500mg" />
                                            </div>
                                            <div className="space-y-3 font-bold italic">
                                                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Duration</label>
                                                <input required value={prescForm.duration} onChange={e => setPrescForm({...prescForm, duration: e.target.value})} className="w-full bg-white border-2 border-slate-100 rounded-2xl p-4 font-black" placeholder="e.g. 7 Days" />
                                            </div>
                                        </div>
                                        <div className="space-y-4 italic font-bold">
                                            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Instructions</label>
                                            <textarea required value={prescForm.instructions} onChange={e => setPrescForm({...prescForm, instructions: e.target.value})} placeholder="e.g. Take twice daily with food..." className="w-full bg-white border-2 border-slate-100 rounded-3xl p-8 text-base font-black text-slate-900 outline-none focus:border-indigo-600 shadow-inner min-h-[150px] italic"></textarea>
                                        </div>
                                        <button disabled={submitting} className="w-full py-8 bg-indigo-600 text-white rounded-[2rem] font-black text-2xl shadow-2xl shadow-indigo-600/30 transition-all flex items-center justify-center gap-6 hover:translate-y-[-4px] italic disabled:opacity-50">
                                            {submitting ? 'Issuing...' : 'Submit Prescription'} <Shield className="w-7 h-7 italic" />
                                        </button>
                                    </form>
                                </div>
                            </div>

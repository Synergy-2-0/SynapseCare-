    ...
    if (loading) return (
        <div className="min-h-screen bg-[var(--bg-base)] flex items-center justify-center">
            <div className="animate-pulse flex flex-col items-center gap-4">
                ...
                <div className="text-xl font-sans text-slate-700 tracking-wide">Synthesizing Clinic View...</div>
            </div>
        </div>
    );
    ...
    return (
        <DashboardLayout>
            ...
            <main className="relative font-sans">
                <div className="absolute top-0 right-0 w-[28rem] h-[28rem] bg-teal-200/20 blur-[120px] -z-10 rounded-full pointer-events-none"></div>
                <div className="absolute bottom-10 left-10 w-72 h-72 bg-indigo-100/25 blur-[110px] -z-10 rounded-full pointer-events-none"></div>

                <header className="flex justify-between items-center mb-10">
                    <div>
                        <h2 className="text-3xl font-bold text-slate-900 tracking-tight leading-tight text-center lg:text-left">Clinical Overview</h2>
                        <p className="copy-description text-sm font-medium mt-2">Start from queue, continue to live consults, then close with prescriptions.</p>
                    </div>
                </header>

                <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
                    <div className="xl:col-span-3 space-y-8">
                        <div className="bg-gradient-to-r from-teal-50 to-cyan-50/70 rounded-[2rem] p-8 lg:p-10 flex justify-between items-center relative overflow-hidden shadow-sm border border-teal-100/40">
                            <div className="z-10">
                                <h2 className="text-3xl font-bold text-teal-900 tracking-tight leading-tight mb-2">Good Morning, {userData?.name || 'Practitioner'}</h2>
                                <p className="text-teal-700/80 text-sm font-medium leading-relaxed">Follow the queue cards below for your next active patient actions.</p>
                                <div className="mt-5 inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/70 border border-teal-100 text-[10px] font-bold uppercase tracking-[0.12em] text-teal-700 backdrop-blur-sm">
                                    Queue
                            ...
                            <div className="absolute right-0 bottom-0 pointer-events-none opacity-90 h-[120%] flex items-end">
                                <div className="w-48 h-48 bg-teal-200/35 rounded-full blur-3xl absolute -right-10 -bottom-10"></div>
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="text-xl font-bold text-slate-800 tracking-tight leading-tight">Weekly Performance</h3>
                                <button className="text-sm font-semibold text-slate-500 hover:text-slate-700 flex items-center gap-1 transition-colors">
                                    This Week <Calendar className="w-4 h-4" />
                                </button>
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                                {[
                                    { label: 'Total Patients', value: stats.activePatients, icon: Users, bg: 'bg-teal-500', text: 'text-white', iconColor: 'text-white', iconBg: 'bg-white/20' },
                                    ...
                                    <div key={i} className={`p-6 rounded-[1.6rem] ${s.bg} flex flex-col items-start justify-center shadow-sm border border-slate-100 hover:-translate-y-1 hover:shadow-md transition-all`}>
                                        <div className={`w-11 h-11 ${s.iconBg} rounded-xl flex items-center justify-center mb-4`}>
                                            <s.icon className={`w-6 h-6 ${s.iconColor}`} />
                                        </div>
                                        <div className={`text-[10px] font-bold uppercase tracking-[0.1em] mb-1 opacity-80 ${s.text}`}>{s.label}</div>
                                        <div className={`text-2xl leading-none font-black ${s.text}`}>{s.value}</div>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="surface-card bg-white overflow-hidden flex flex-col mt-8 shadow-sm border border-slate-100 rounded-[2rem]">
                            <div className="p-7 border-b border-slate-100 flex justify-between items-center bg-gradient-to-r from-slate-50/80 to-white">
                                <div>
                                    <h3 className="text-xl font-bold text-slate-900 tracking-tight leading-tight">Waitlist Queue</h3>
                                    <p className="copy-description text-xs font-medium mt-1.5">Open any row to view patient context and continue treatment flow.</p>
                                </div>
                                <button className="px-4 py-2 bg-teal-50 text-teal-700 rounded-xl text-xs font-bold hover:bg-teal-100 transition-colors shadow-sm border border-teal-100">
                                    Open Queue
                                </button>
                            </div>
                            <div className="overflow-x-auto">
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="border-b border-slate-100 bg-slate-50/70 text-[10px] uppercase font-bold tracking-[0.1em] text-slate-400">
                                            <th className="p-6 pl-8">Patient</th>
                                    ...
                                    <tbody className="divide-y divide-slate-100">
                                        {appointments.filter(a => a.status !== 'PENDING').slice(0, 5).map((appt, i) => (
                                            <tr key={i} className="hover:bg-teal-50/20 transition-colors group cursor-pointer" onClick={() => { setSelectedAppointment(appt); setIsDrawerOpen(true); }}>
                                                <td className="p-6 pl-8">
                                                    <div className="flex items-center gap-4">
                                                        <div className="w-10 h-10 rounded-full overflow-hidden bg-slate-100 flex-shrink-0 shadow-sm border border-slate-200">
                                                            <img src={`https://ui-avatars.com/api/?name=P${appt.patientId}&background=random&color=fff`} alt="P" />
                                                        </div>
                                                        <span className="font-bold text-slate-800">Patient ID #{appt.patientId}</span>
                                                    </div>
                                                </td>
                                                <td className="p-6">
                                                ...
                                                <td className="p-6">
                                                    <div className="flex flex-col">
                                                        <span className="text-xs font-bold leading-tight text-slate-700">{appt.date}</span>
                                                        <span className="text-[10px] font-medium leading-tight text-slate-500 mt-1">{appt.time}</span>
                                                    </div>
                                                </td>
                                                <td className="p-6">
                                                    <div className="flex items-center gap-2">
                                                        <div className={`w-2 h-2 rounded-full ${appt.status === 'PAID' ? 'bg-teal-500' : 'bg-amber-500'} animate-pulse`} />
                                                        <span className="text-[10px] font-black uppercase tracking-[0.12em] text-slate-600">{appt.status}</span>
                                                    </div>
                                                </td>
                                                <td className="p-6 pr-8 text-right">
                                                    {(appt.status === 'CONFIRMED' || appt.status === 'PAID') && (
                                                        <button
                                                            ...
                                                            className="w-10 h-10 rounded-xl bg-slate-50 border border-slate-200 text-slate-400 flex items-center justify-center hover:bg-teal-600 hover:text-white hover:border-teal-600 transition-all ml-auto group-hover:shadow-lg group-hover:shadow-teal-500/20"
                                                            title="Create Post-Session Prescription"
                                                        >
                                            ...
                                            <tr>
                                                <td colSpan="5" className="p-12 text-center">
                                                    <div className="flex flex-col items-center opacity-40">
                                                        ...
                                                        <p className="text-sm font-medium text-slate-400">No active sessions in queue right now.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        )}
                    ...
                    <div className="space-y-8">
                        <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm relative overflow-hidden">
                            <div className="flex justify-between items-center mb-8">
                                <h4 className="text-xl font-bold text-slate-900 tracking-tight leading-tight">Clinic Calendar</h4>
                            ...
                            <div className="flex justify-between text-center pb-6">
                                {[0, 1, 2, 3, 4, 5, 6].map((i) => {
                                    ...
                                    return (
                                        <div key={i} className={`flex flex-col items-center gap-2 p-3 rounded-2xl transition-all ${isCurrentDay ? 'bg-teal-600 text-white shadow-md shadow-teal-500/20 scale-105' : 'text-slate-400 hover:bg-slate-50 border border-transparent hover:border-slate-100'}`}>
                            ...
                            <div className="border-t border-slate-50 pt-8">
                                <h4 className="text-lg font-bold text-slate-900 tracking-tight leading-tight mb-6">Operational Health</h4>
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="p-4 bg-teal-50/40 rounded-[1.5rem] border border-teal-100/30 text-center group hover:bg-teal-50 transition-colors">
                                    ...
                                    <div className="p-4 bg-indigo-50/40 rounded-[1.5rem] border border-indigo-100/30 text-center group hover:bg-indigo-50 transition-colors">
                        ...
                        <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-[2rem] p-8 text-white relative overflow-hidden shadow-lg shadow-slate-200">
                            <div className="absolute top-0 right-0 w-32 h-32 bg-teal-500/20 blur-[50px] rounded-full" />
                            <h4 className="text-xl font-bold tracking-tight leading-tight mb-2 relative z-10">Weekly Patient Flow</h4>
                            <p className="text-[11px] font-medium text-white/60 mb-6 relative z-10">Higher bars indicate heavier consultation load for that day.</p>
